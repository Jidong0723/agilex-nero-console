(() => {
  ...
  function renderDataset() {
    ...
    picoText("dataset-sample-kinds", `${data.static_frame_count ?? 0} / ${data.action_frame_count ?? 0}`); picoText("dataset-rejected", data.rejected_samples ?? data.dropped_frames ?? 0); picoText("dataset-dropped", data.dropped_frames ?? 0); picoText("dataset-bytes", datasetBytes(data.bytes_written));
    ...
    const feedback = data.last_error || (recording && reasons) || (recording ? ((data.static_frame_count || 0) > 0 && !(data.action_frame_count || 0) ? "正在采集静态观测；PICO 未移动时动作字段会为空。" : `Episode ${String(data.episode_index || "").padStart(6, "0")} 正在记录。`) : data.deleted ? "失败 Episode 已安全删除。开始后会实时显示数据量、帧率和实际记录内容。" : data.status === "completed" ? "Episode 已保存。开始后会实时显示数据量、帧率和实际记录内容。" : "开始后会实时显示数据量、帧率和实际记录内容。");
